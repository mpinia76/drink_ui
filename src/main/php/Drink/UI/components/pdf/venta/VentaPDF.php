...
namespace Drink\UI\components\pdf\venta;
...
class VentaPDF extends RastyComponent{
	...
	protected function parseXTemplate(XTemplate $xtpl){
		...
		if( !empty($venta )){
			...
			$xtpl->assign( "fecha", DrinkUIUtils::formatDateTimeToView($venta->getFecha()) );
			$observaciones = ($venta->getObservaciones())?' - '.$venta->getObservaciones():'';
			$xtpl->assign( "cliente", $venta->getCliente().$observaciones);
			$xtpl->assign( "direccion", $venta->getCliente()->getDireccion() );
			...
			foreach ($venta->getDetalles() as $detalle) {
				if (!empty($detalle->getCombo())) {
					...
					if(!in_array($detalle->getCombo()->getOid(), $combos)){
						...
						$xtpl->assign( "precio", DrinkUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad ) );
						$xtpl->assign( "subtotal", DrinkUIUtils::formatMontoToView( $detalle->getCombo()->getPrecio()*$cantidad  ) );
			...
			$xtpl->assign( "total", DrinkUIUtils::formatMontoToView( $total ) );
			...
			foreach ($venta->getDevoluciones() as $devolucion) {
				...
				$htmlItem .='<tr><td>'.$devolucion->getProducto().'</td><td style="text-align: right;">'.$devolucion->getCantidad().'</td><td style="text-align: right;">'.DrinkUIUtils::formatMontoToView( $devolucion->getPrecioUnitario() ).'</td><td style="text-align: right;">'.DrinkUIUtils::formatMontoToView( $devolucion->getSubtotal() ).'</td></tr>';
	...
	<table cellspacing="0" cellpadding="0" border="1">
		...
		<tr><td colspan="4"></td></tr>
		<tr><td colspan="4"></td></tr>
			<tr><td style="text-align: right; font-weight: bold;" colspan="3">'.$this->localize( "venta.detalle.subtotal" ).'</td><td style="text-align: right;font-weight: bold;">'.DrinkUIUtils::formatMontoToView( $venta->getMontoDevolucion() ).'</td></tr>
			...
			if ($venta->getMontoActualizado()) {
				$xtpl->assign( 'htmlMontoActualizado','<tr><td style="text-align: right; font-weight: bold;" colspan="3" width="616px">'.$this->localize('venta.montoActualizado').'</td><td style="text-align: right;font-weight: bold;" width="100px">'.DrinkUIUtils::formatMontoToView($venta->getMontoActualizado()),'</td></tr>');
			}
			$xtpl->assign( "totales", DrinkUIUtils::formatMontoToView( $venta->getMonto() ) );
			$xtpl->assign( "pagado", DrinkUIUtils::formatMontoToView( $venta->getMontoPagado() ) );
			$xtpl->assign( "adeudado", DrinkUIUtils::formatMontoToView( $venta->getMontoDebe() ) );
