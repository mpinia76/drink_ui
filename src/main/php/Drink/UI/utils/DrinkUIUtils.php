...
class DrinkUIUtils {
	 ...
	 */
	public static function setVendedorSession($vendedor_oid) {

		$appName = RastyConfig::getInstance()->getAppName();

        $_SESSION [$appName]["vendedor_oid"] = $vendedor_oid;


    }

	public static function getVendedorSession() {

    	$appName = RastyConfig::getInstance()->getAppName();

    	$data = RastyUtils::getParamSESSION($appName);

		if( isset( $data['vendedor_oid']) )
			$vendedor_oid = $data['vendedor_oid'];
		else
			$vendedor_oid = DrinkUtils::DRINK_VENDEDOR_DEFAULT;
	...
	public static function agregarDetalleVentaSession(DetalleVenta $detalle) {
		...
		if ($detalle->getCombo()) {
			...
			$detallejson["combo_precio"] = $detalle->getCombo()->getPrecio();
	    ...
	    $detallejson["subtotal"] = round($detalle->getCantidad() * $detalle->getPrecioUnitario(),2);
	    $detallejson["ganancia"] = round(($detalle->getCantidad() * $detalle->getPrecioUnitario())-$detalle->getCantidad() * $detalle->getCosto(),2);
		if($existe){
	...
	public static function agregarDevolucionVentaSession(DevolucionVenta $devolucion) {
	    ...
	    $devolucionjson["subtotal"] = round($devolucion->getCantidad() * $devolucion->getPrecioUnitario(),2);
	...
	public static function agregarProductoComboSession(ProductoCombo $producto) {
	    ...
	    $productojson["subtotal"] = round($producto->getCantidad() * $producto->getPrecioUnitario(),1);
