...
/**
 * 
 * Finder para parametros.
 * 
 * @author Marcos
 * @since 16/07/2018
 */
class ParametroFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIParametroCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIParametroService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIParametroService()->get( $code );
	}
