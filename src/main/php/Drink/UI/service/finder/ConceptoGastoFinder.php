...
use Drink\UI\components\filter\model\UIConceptoGastoCriteria;
...
/**
 * 
 * Finder para conceptoGastos.
 * 
 * @author Marcos
 * @since 12/03/2018
 */
class ConceptoGastoFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIConceptoGastoCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIConceptoGastoService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIConceptoGastoService()->get( $code );
	}
