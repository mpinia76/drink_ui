...
use Drink\UI\components\filter\model\UITarjetaCriteria;
...
/**
 * 
 * Finder para tarjetas.
 * 
 * @author Marcos
 * @since 02/04/2018
 */
class TarjetaFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UITarjetaCriteria();
		$uiCriteria->setNro( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUITarjetaService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUITarjetaService()->get( $code );
	}
