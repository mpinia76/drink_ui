<?php
namespace Drink\UI\service\finder;

use Drink\UI\components\filter\model\UIProveedorCriteria;
...
/**
 * 
 * Finder para proveedors.
 * 
 * @author Marcos
 * @since 10/07/2020
 */
class ProveedorFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIProveedorCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		
		
		$uiCriteria->addOrder("nombre", "ASC");
		return UIServiceFactory::getUIProveedorService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIProveedorService()->get( $code );
	}
	...
	public function getAttributes(){
		return array("nombre", "razonSocial");		
	}
	...
	public function getAttributesCallback(){
		return array("oid", "nombre", "razonSocial");		
	}
	...
	function getOnclickAdd(){
		...
		return "javascript: gotoLink('".LinkBuilder::getPageUrl( "ProveedorAgregar", $params)."')";*/
		return"";
	}
}
?>