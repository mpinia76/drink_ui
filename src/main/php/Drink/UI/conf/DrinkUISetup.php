...
class DrinkUISetup {
	 ...
	 */
	public static function initialize( $appName = "drink_ui"){
...
//		$_SESSION['LAST_ACTIVITY'] = time();
	...
	private static function initializeCore(){
		...
		DrinkConfig::getInstance()->setDbName("cose_drink");
		
		DrinkConfig::getInstance()->initialize();
		DrinkConfig::getInstance()->initLogger(dirname(__DIR__) . "/conf/log4php.xml");
	...
	private static function initializeUI( $appName ){
		...
		Logger::log("Logger drink ui configurado!");
		...
		$rastyLoader->loadXml(
				...
				RastyConfig::getInstance()->getAppPath() . "src/main/php/Drink/UI/",
		...
		//SorteosUIConfig::getInstance()->initialize("DrinkMetroLayout");
