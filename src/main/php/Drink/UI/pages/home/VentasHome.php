<?php
namespace Drink\UI\pages\home;

use Datetime;
use Drink\UI\pages\DrinkPage;

use Drink\UI\components\filter\model\UIEmpleadoCriteria;
...
class VentasHome extends DrinkPage{
	...
	public function __construct(){
		...
		/*if( DrinkUIUtils::isCajaSelected() ){
			$caja = UIServiceFactory::getUICajaService()->get(DrinkUIUtils::getCaja()->getOid());
			$this->setCaja( $caja );
		}*/
	...
	public function parseStatsVentas(XTemplate $xtpl){
		...
		$xtpl->assign("monto_ventas", DrinkUIUtils::formatMontoToView($totalesDia["monto"]) );
		...
		$xtpl->assign("monto_ventas_mes", DrinkUIUtils::formatMontoToView($totalesMes["monto"]) );
	...
	public function parseDebitosCreditos( XTemplate $xtpl){
		...
		foreach ($debitosCreditos as $informeDebitoCredito) {
			...
			$xtpl->assign("titulo", DrinkUIUtils::formatDateToView( $informeDebitoCredito->getFechaVencimiento()) );

			if($informeDebitoCredito->getDebito()){
				$xtpl->assign("subtitulo", DrinkUIUtils::formatMontoToView($informeDebitoCredito->getDebito()) );
				$xtpl->assign("descripcion", "Débito" );
			}else{
				$xtpl->assign("subtitulo", DrinkUIUtils::formatMontoToView($informeDebitoCredito->getCredito()) );
	...
	public function parseSaldos(XTemplate $xtpl){
		...
		//if( DrinkUIUtils::isCajaSelected() ){
			...
			$xtpl->assign("saldo_caja", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesDia(new Datetime())) );
			$xtpl->assign("saldo_ventas", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoVentaService()->getTotalesDia(new Datetime())) );
			$xtpl->assign("saldo_gastos", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoGastoService()->getTotalesDia(new Datetime())) );

			$xtpl->assign("saldo_cajaTarjeta", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesTarjetasDia(new Datetime())) );
			$xtpl->assign("saldo_cajaCtaCte", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIMovimientoCajaService()->getTotalesCtasCtesDia(new Datetime())) );
		...
		/*}else{
			$xtpl->assign("saldo_caja", DrinkUIUtils::formatMontoToView( 0) );
		}*/

		/*$xtpl->assign("saldo_cajaChica", DrinkUIUtils::formatMontoToView( $this->getCajaChica()->getSaldo() ) );
		$xtpl->assign("linkMovimientosCajaChica", $this->getLinkMovimientosCajaChica());

		$xtpl->assign("saldo_bancos", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUIBancoService()->getSaldoBancos() ) );
		$xtpl->assign("linkMovimientosBanco", $this->getLinkMovimientosBanco());

		$xtpl->assign("saldo_ctasctes", DrinkUIUtils::formatMontoToView( UIServiceFactory::getUICuentaCorrienteService()->getSaldoCtasCtes() ) );
	*/
	}
	...
	public function parseGastos( XTemplate $xtpl){
		...
		foreach ($gastos as $gasto) {
			$xtpl->assign("titulo", DrinkUIUtils::formatDateToView( $gasto->getFechaVencimiento()) );
			$xtpl->assign("subtitulo", DrinkUIUtils::formatMontoToView($gasto->getMonto()) );
